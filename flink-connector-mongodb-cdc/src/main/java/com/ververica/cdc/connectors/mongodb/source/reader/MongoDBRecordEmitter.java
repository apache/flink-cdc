...
public final class MongoDBRecordEmitter<T>
        implements RecordEmitter<SourceRecord, T, MongoDBSplitState> {

    private final DebeziumDeserializationSchema<T> deserializationSchema;
    private final MongoDBSourceReaderMetrics sourceReaderMetrics;
    private final OutputCollector<T> outputCollector;

    public MongoDBRecordEmitter(
            DebeziumDeserializationSchema<T> deserializationSchema,
            MongoDBSourceReaderMetrics sourceReaderMetrics) {
    ...
    public void emitRecord(
            SourceRecord element, SourceOutput<T> output, MongoDBSplitState splitState)
            throws Exception {
        if (isHeartbeatEvent(element)) {
            if (splitState.isStreamSplitState()) {
                ...
                splitState.asStreamSplitState().updateOffset(resumeToken, timestamp);
            }
        } else {
            if (splitState.isStreamSplitState()) {
                BsonDocument resumeToken = getResumeToken(element);
                BsonTimestamp clusterTime = getClusterTimeFromDataRecord(element);
                splitState.asStreamSplitState().updateOffset(resumeToken, clusterTime);
            }
