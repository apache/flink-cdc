...
@Internal
public class MongoDBSourceConfigFactory implements Serializable {
    ...
    private Boolean scanNewlyAddedCollectionEnabled =
    ...
    /**
     * scan.newly-added-collection.enabled
     *
     * <p>Whether the should scan the newly added collections or not. Default: false
     */
    public MongoDBSourceConfigFactory scanNewlyAddedCollectionEnabled(
    ...
    public MongoDBSourceConfig createConfig(int subtaskId) {
        return new MongoDBSourceConfig(
                ...
                scanNewlyAddedCollectionEnabled,
                chunkSizeMB);
    }

    /** Creates a new {@link MongoDBChangeStreamConfig} for the stream read task. */
    public MongoDBChangeStreamConfig createChangeStreamConfig(
        ...
        if (collectionList != null) {
            ...
            if (isIncludeListExplicitlySpecified(collectionList, discoveredCollections)) {
                changeStreamConfig =
                        MongoDBChangeStreamConfig.collection(
                                CollectionId.parse(discoveredCollections.get(0)));
            } else {
                ...
                if (databaseList != null) {
                    if (isIncludeListExplicitlySpecified(databaseList, discoveredDatabases)) {
                        changeStreamConfig =
                                MongoDBChangeStreamConfig.database(
                                        discoveredDatabases.get(0), namespaceRegex);
                    } else {
                        Pattern databaseRegex = includeListAsFlatPattern(databaseList);
                        changeStreamConfig =
                                MongoDBChangeStreamConfig.deployment(databaseRegex, namespaceRegex);
                    }
                } else {
                    changeStreamConfig = MongoDBChangeStreamConfig.deployment(null, namespaceRegex);
                }
            }
        } else if (databaseList != null) {
            if (isIncludeListExplicitlySpecified(databaseList, discoveredDatabases)) {
                return MongoDBChangeStreamConfig.database(discoveredDatabases.get(0));
            } else {
                Pattern databaseRegex = includeListAsFlatPattern(databaseList);
                changeStreamConfig = MongoDBChangeStreamConfig.deployment(databaseRegex);
            }
        } else {
            // Watching all changes on the cluster
            changeStreamConfig = MongoDBChangeStreamConfig.deployment();
        }

        changeStreamConfig.setBatchSize(batchSize);
