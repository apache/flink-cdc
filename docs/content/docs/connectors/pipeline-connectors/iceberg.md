...
Metadata scan tasks carry the effective Hadoop configuration alongside the original FileIO implementation and properties in their JSON representation. This preserves `hadoop.conf.*` when the downstream metadata reader reconstructs a scan split, as well as during candidate file listing. FileIOs without Hadoop configuration retain their native serialization.
