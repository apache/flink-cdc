...
| `CHECK` | Validate that every upstream column can be contained by the target table and that its primary key matches, without issuing any DDL | Fails the job; the table must be created externally | Any incompatibility, read failure, or missing capability fails the job with an aggregated error |
...
Expansion never realigns table keys. The pipeline's primary key is compared against the existing target table's primary key, ignoring partition columns on both sides because sinks such as Paimon store them inside the stored primary key, and a mismatch is reported as an incompatibility: `CHECK` and `EXPAND` fail the job, while `TRY_EXPAND` skips expansion entirely - it issues no DDL against a table whose keys can never match - and hands the `CreateTableEvent` to the sink, so only a warning is logged and the connector's own key validation has the last word. Use `CHECK` or `EXPAND` when a key mismatch must be fatal. Partition keys are only compared when the pipeline itself declares them (through a `PARTITION BY` transform or a source that reports partitioning): most sources do not, so an externally partitioned target table is not reported as incompatible.
