...
| `schema-operator.rpc-timeout` | The timeout time for SchemaOperator to wait downstream SchemaChangeEvent applying finished, the default value is 3 minutes. For Flink 2.x with wide tables (many columns), ensure `pekko.ask.timeout` in cluster `flink-conf.yaml` is at least 5 minutes; otherwise SchemaChangeRequest RPC may time out (Flink default 120s). Flink CDC CLI auto-sets it when absent; for existing session clusters, add to `$FLINK_HOME/conf/flink-conf.yaml`.                                                                                                                                                                                                 | optional          |
