...
| `new_db_6.table_17`| `new_db_6.table_([1-9]|1[0-6])`           | `new_db_6.table_$1_suffix`          | `new_db_6.table_1_suffix7`                 | `new_db_6.table_17`（不匹配）         |
...
- 第一行展示的是一位数字的 source（`table_1`）。此时两个版本的结果恰好一致，因为没有多余的尾部
  可以拼到 sink-table 名后。
- 第二、三行展示 `$N` 反向引用的情形：3.6.0 中捕获值被截断为首位数字（`1`），剩余字符被拼到
  `_suffix` 字面量之后；3.7.0 中捕获到完整的多位数字，并且当正则无法完整匹配 source 表 id 时
  （例如 `table_17`），该 source 不再被路由。
- 第四、五行展示不带 `$N` 反向引用的情形：3.6.0 中每个匹配的 source 都会拼上自己的尾部
  （`merged3`、`merged4` 等），相当于一个 source 表对应一个 sink 表；3.7.0 中所有匹配的 source
  会汇聚到规则中声明的同一个 sink-table。

## 对现有任务的兼容性影响

本次行为变化只会影响那些 `source-table` 规则中**包含可能匹配多字符的捕获组**（例如
`([1-9]|1[0-6])`、`(db_[0-9]+)`、`(\w{2,4})` 等）的任务。仅使用单字符捕获组、字面量表名，
或者仅依赖 `replace-symbol` 选项的规则不受影响。

对于**确实**会受影响的任务，从 3.6.0 升级到 3.7.0 后可能观察到以下两类行为变化：

- **原本被（错误）路由的 source 表，会落到不同的 sink-table 上。**
  - *使用 `$N` 反向引用时：* 捕获值由原来的截断子串变为完整的多字符子串（例如从
    `..._table_1_suffix3` 变为 `..._table_13_suffix`）。
  - *不使用 `$N` 反向引用时：* 原本每个被匹配的 source 都会被拼上自己的尾部；
    修正后所有匹配的 source 会汇聚到规则中声明的同一个 sink-table（例如从 `table_merged3` /
    `table_merged4` / ... 全部变为 `table_merged`）。

  如果已经在下游创建过旧（错误）名字的表，它们在升级后将不再是 pipeline 的目标。请决定是
  重命名受影响的下游表，还是把已有数据迁移到新的（修正后的）sink 表中。

- **原本只能被“部分匹配”的 source 表，不再被该规则路由。** 它们会落入“未匹配”的默认分支，
  保持原来的 source 表 id。这通常是符合预期的，但如果某个任务曾依赖 3.6.0 的“部分匹配”错误
  路由来工作，该任务需要被显式修正。

总结一下：升级之前，请审查所有 route 规则，定位其中**包含多字符捕获组的 source-table 正则**，
确认其匹配集合正是你期望的；如果有任务曾依赖 3.6.0 的错误路由行为，请决定是重命名受影响的下游表，
还是把已有数据迁移到新的（修正后的）sink 表中。
