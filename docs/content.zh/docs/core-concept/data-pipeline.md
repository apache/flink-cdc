...
| `flink-conf`                               | 用于配置 [Flink 相关参数](https://nightlies.apache.org/flink/flink-docs-master/docs/deployment/config/)。<br/>配置优先级：命令行参数（CLI arguments）高于 pipeline.yaml 高于 Flink 默认配置 config.yaml。同时支持扁平键值对与嵌套 YAML 结构。                                                                                                                                                                                                                                                  | optional          |
