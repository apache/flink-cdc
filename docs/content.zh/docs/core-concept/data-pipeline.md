...
| `schema-operator.rpc-timeout` | SchemaOperator 等待下游 SchemaChangeEvent 应用完成的超时时间，默认值是 3 分钟。Flink 2.x 同步宽表（列多）时，需确保集群配置中 `pekko.ask.timeout` 至少为 5 分钟，否则 SchemaChangeRequest RPC 可能超时（Flink 默认 120 秒）。Flink CDC CLI 会在配置缺失时自动设置；已有 session 集群请将该项加入 `$FLINK_HOME/conf/flink-conf.yaml`。                                                                                                                                                                                                                                                                   | optional          |
