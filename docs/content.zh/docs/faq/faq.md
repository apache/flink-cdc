...
## Pipeline / Schema Operator 常见问题

### Q1: Flink 2.x 作业失败，报错 "Failed to send request to coordinator: SchemaChangeRequest" 和约 2 分钟后超时的 TimeoutException，怎么办？

这是因为 SchemaOperator 通过 RPC 向 JobManager 上的 SchemaCoordinator 发送 schema 变更请求（如宽表的 CreateTableEvent）。Flink 2.x 使用 Pekko，默认 `pekko.ask.timeout` 为 120 秒。对于宽表（列很多）或 Paimon 等将 schema 应用到远程存储（如 S3）时，操作可能超过 2 分钟。

**解决方案：**

1. **Flink CDC CLI**（推荐）：CLI 现会在 Flink 配置未包含该项时自动设置 `pekko.ask.timeout: 5 min`。重启集群或使用 application 模式使配置生效。

2. **已有 session 集群**：在 JobManager 和 TaskManager 节点的 `$FLINK_HOME/conf/flink-conf.yaml` 中添加，然后重启集群：
   ```yaml
   pekko.ask.timeout: 5 min
   ```

3. **命令行覆盖**：`flink-cdc.sh cdc/your-pipeline.yaml -D pekko.ask.timeout=5 min`（适用于 application 模式部署）