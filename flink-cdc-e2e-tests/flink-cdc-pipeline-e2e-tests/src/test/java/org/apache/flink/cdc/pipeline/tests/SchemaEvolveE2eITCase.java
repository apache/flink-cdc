...
public class SchemaEvolveE2eITCase extends PipelineTestEnvironment {
    ...
    public void testSchemaTryEvolveWithException() throws Exception {
        testGenericSchemaEvolution(
                ...
                Arrays.asList(
                        "Failed to apply schema change AddColumnEvent{tableId=%s.members, addedColumns=[ColumnWithPosition{column=`gender` TINYINT JdbcRawDataType{jdbcType=5, typeName='TINYINT', length=-1, scale=null}, position=AFTER, existedColumnName=age}]}, but keeps running in tolerant mode. Caused by: UnsupportedSchemaChangeEventException{applyingEvent=AddColumnEvent{tableId=%s.members, addedColumns=[ColumnWithPosition{column=`gender` TINYINT JdbcRawDataType{jdbcType=5, typeName='TINYINT', length=-1, scale=null}, position=AFTER, existedColumnName=age}]}, exceptionMessage='Rejected schema change event since error.on.schema.change is enabled.', cause='null'}",
                        "Failed to apply schema change AlterColumnTypeEvent{tableId=%s.members, typeMapping={age=DOUBLE JdbcRawDataType{jdbcType=8, typeName='DOUBLE', length=-1, scale=null}}, oldTypeMapping={age=INT JdbcRawDataType{jdbcType=4, typeName='INT', length=-1, scale=null}}}, but keeps running in tolerant mode. Caused by: UnsupportedSchemaChangeEventException{applyingEvent=AlterColumnTypeEvent{tableId=%s.members, typeMapping={age=DOUBLE JdbcRawDataType{jdbcType=8, typeName='DOUBLE', length=-1, scale=null}}, oldTypeMapping={age=INT JdbcRawDataType{jdbcType=4, typeName='INT', length=-1, scale=null}}}, exceptionMessage='Rejected schema change event since error.on.schema.change is enabled.', cause='null'}",
