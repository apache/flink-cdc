...
public class BinlogOffsetUtils {
     ...
     *   <li>TIMESTAMP: set to earliest, use SimpleBinlogReadTask to find binlog position by specified timestamp, if not found,
     *   start from earliest binlog file
    ...
    public static BinlogOffset seekBinlogPositionByTimestamp(BinlogOffset binlogOffset, StatefulTaskContext statefulTaskContext) {
        List<BinlogOffset> binlogOffsetList = DebeziumUtils.allBinlogFilesAndOffset(statefulTaskContext.getConnection());
        ...
        for (BinlogOffset file : binlogOffsetList) {
            ...
            SimpleBinlogReadTask simpleBinlogReadTask  =
                    new SimpleBinlogReadTask(statefulTaskContext.getConnectorConfig(),
            ...
            OffsetContext.Loader<MySqlOffsetContext> loader = new MySqlOffsetContext.Loader(statefulTaskContext.getConnectorConfig());
            try {
                simpleBinlogReadTask.execute(new SimpleBinlogChangeEventSourceContextImpl(), loader.load(currentBinlogOffset.getOffset()));
                if (StringUtils.isNotBlank(seekedBinlogOffset.get(BinlogOffset.BINLOG_FILENAME_OFFSET_KEY))) {
                    LOG.info("fetch binlog position success! filename: {}, position: {}",
                    ...
                    return BinlogOffset.ofBinlogFilePosition(seekedBinlogOffset.get(BinlogOffset.BINLOG_FILENAME_OFFSET_KEY),
                            Long.valueOf(seekedBinlogOffset.get(BinlogOffset.BINLOG_POSITION_OFFSET_KEY)));
                }
            } catch (Exception e) {
                throw new FlinkRuntimeException("Execute binlog read task for seek binlog position by timestamp fail", e);
            }
        }
