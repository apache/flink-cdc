...
/** Flink sink wrapper for Milvus event writer. */
public class MilvusEventSink implements Sink<Event> {

    private final MilvusDataSinkConfig config;

    public MilvusEventSink(MilvusDataSinkConfig config) {
        this.config = config;
    }

    @Override
    public SinkWriter<Event> createWriter(InitContext context) throws IOException {
        return new MilvusEventWriter(
                config,
                new MilvusSinkMetrics(InternalSinkWriterMetricGroup.wrap(context.metricGroup())));
    }
