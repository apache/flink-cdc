...
public class ColumnDefinitionParserListener extends BaseParserListener {
    ...
    public ColumnDefinitionParserListener(
            final TableEditor tableEditor, final ColumnEditor columnEditor) {
    ...
    private void resolveColumnDataType(PlSqlParser.DatatypeContext ctx) {
        ...
        if (ctx.native_datatype_element() != null) {
            ...
            if (ctx.native_datatype_element().NUMERIC() != null
            ...
            } else if (ctx.native_datatype_element().INT() != null
                    || ctx.native_datatype_element().INTEGER() != null
                    || ctx.native_datatype_element().SMALLINT() != null) {
                // NUMERIC and DECIMAL types have by default zero scale
                columnEditor.jdbcType(Types.INTEGER).type("INT");
                ...
                columnEditor.jdbcType(Types.DATE).type("DATE");
            ...
            if (!ctx.expression().isEmpty()) {
                columnEditor.length(Integer.parseInt((ctx.expression(0).getText())));
            }
        } else if (ctx.INTERVAL() != null
                && ctx.DAY() != null
                && ctx.TO() != null
            ...
            for (final PlSqlParser.ExpressionContext e : ctx.expression()) {
                if (e.getSourceInterval().startsAfter(ctx.TO().getSourceInterval())) {
                    ...
                    columnEditor.length(Integer.parseInt(e.getText()));
                }
            }
            if (!ctx.expression().isEmpty()) {
                columnEditor.length(Integer.parseInt((ctx.expression(0).getText())));
    ...
    private void setPrecision(
        ...
        columnEditor.length(Integer.parseInt(precisionPart.numeric(0).getText()));
    }
