...
/** Tests for {@link OracleRowIdExtractor}. */
class OracleRowIdExtractorTest {
    ...
    void testExtractRowIdFromAfterStruct() {
        SourceRecord record = createDataChangeRecord(LOWER_ROWID, null);

        assertThat(OracleRowIdExtractor.extractRowId(record))
                .isPresent()
                .get()
    ...
    void testExtractRowIdFromBeforeStruct() {
        SourceRecord record = createDataChangeRecord(null, UPPER_ROWID);
    ...
    void testExtractRowIdReturnsEmptyWhenMissing() {
        SourceRecord record = createDataChangeRecord(null, null);

        assertThat(OracleRowIdExtractor.extractRowId(record)).isEmpty();
    }
    ...
    @Test
    void testExtractRowIdReturnsEmptyWhenBeforeFieldIsMissing() {
        Schema sourceSchema =
        ...
        Struct value =
                new Struct(valueSchema)
                        ...
                        .put("after", new Struct(rowSchema).put("ID", 1).put("ROWID", null));
    ...
    private SourceRecord createDataChangeRecord(String afterRowId, String beforeRowId) {
        return createDataChangeRecord(afterRowId, beforeRowId, null);
    }
    ...
    private SourceRecord createDataChangeRecord(
        ...
        if (beforeRowId != null) {
            value.put("before", new Struct(rowSchema).put("ID", 1).put("ROWID", beforeRowId));
        }
        if (afterRowId != null) {
            value.put("after", new Struct(rowSchema).put("ID", 1).put("ROWID", afterRowId));
        }

        ConnectHeaders headers = new ConnectHeaders();
        if (headerRowId != null) {
            headers.add("ROWID", headerRowId, null);
        }
