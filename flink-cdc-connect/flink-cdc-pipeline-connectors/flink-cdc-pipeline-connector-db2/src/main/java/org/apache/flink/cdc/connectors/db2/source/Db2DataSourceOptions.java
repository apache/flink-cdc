...
public class Db2DataSourceOptions {
    ...
    public static final ConfigOption<String> USERNAME =
            ConfigOptions.key("username")
                    ...
                    .noDefaultValue()
                    .withDescription(
                            "Name of the Db2 database user to use when connecting to the Db2 database server.");
    ...
    public static final ConfigOption<String> DATABASE =
            ConfigOptions.key("database")
                    .stringType()
                    .noDefaultValue()
                    .withDescription("Database name of the Db2 database server.");
            ...
            ConfigOptions.key("tables")
                    ...
                    .withDescription(
                            "Table names of the Db2 tables to monitor. Regular expressions are supported. "
                                    ...
                                    + "It is important to note that the dot (.) is treated as a delimiter for schema and table names. "
                                    ...
                                    + "eg. DB2INST1.\\.*, DB2INST1.user_table_[0-9]+, DB2INST1.(APP|WEB)_ORDER_\\.*");
    ...
    public static final ConfigOption<Integer> CONNECT_MAX_RETRIES =
            ConfigOptions.key("connect.max-retries")
                    ...
                    .defaultValue(3)
                    .withDescription(
                            "The max retry times that the connector should retry to build Db2 database server connection.");
            ...
            ConfigOptions.key("chunk-key.even-distribution.factor.upper-bound")
                    ...
                    .withDescription(
                            "The upper bound of chunk key distribution factor. The distribution factor is used to determine whether the"
                                    ...
                                    + " and the query Db2 for splitting would happen when it is uneven."
            ...
            ConfigOptions.key("tables.exclude")
                    ...
                    .withDescription(
                            "Table names of the Db2 tables to exclude. Regular expressions are supported. "
                                    + "Every exclude pattern uses the format \"schema.table\". "
                                    + "Multiple entries are separated by a comma (,); escape a comma with a backslash when it is part of a regular expression. "
                                    + "It is important to note that the dot (.) is treated as a delimiter for schema and table names. "
                                    + "If there is a need to use a dot (.) in a regular expression to match any character, "
                                    + "it is necessary to escape the dot with a backslash."
