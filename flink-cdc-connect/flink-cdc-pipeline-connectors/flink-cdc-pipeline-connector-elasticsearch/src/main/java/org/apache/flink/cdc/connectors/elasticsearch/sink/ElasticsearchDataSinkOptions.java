...
public class ElasticsearchDataSinkOptions {
    ...
    public static final ConfigOption<String> SHARDING_KEY =
            ConfigOptions.key("sharding.suffix.key")
                    ...
                    .defaultValue("")
                    .withDescription(
                            "Sharding suffix key for each table, allow setting sharding suffix key for multiTables.Default sink table name is test_table_${suffix_key}.Tables are separated by ';'.For example, we can set sharding.suffix.key by 'table1:col1;table2:col2'");
