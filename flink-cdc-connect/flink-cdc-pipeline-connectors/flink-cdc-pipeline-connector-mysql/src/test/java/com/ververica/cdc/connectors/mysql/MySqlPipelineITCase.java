...
public class MySqlPipelineITCase extends MySqlSourceTestBase {
    ...
    public void test() throws Exception {
        ...
        executeDataStreamJob();
    }

    private void executeDataStreamJob() throws Exception {
        ...
        try (Connection connection = inventoryDatabase.getJdbcConnection();
            ...
            statement.execute(
                    String.format(
                            "ALTER TABLE `%s`.`products` ADD COLUMN `col1` VARCHAR(45) NULL AFTER `weight`, ADD COLUMN `col2` VARCHAR(45) NULL AFTER `desc1`;",
                            inventoryDatabase.getDatabaseName()));
            statement.execute(
                    String.format(
