...
/** Metrics for LanceDB sink writer. */
class LanceDbSinkMetrics {

    static final LanceDbSinkMetrics NOOP = new LanceDbSinkMetrics(null);
    ...
    private final Counter reopenCount;

    private volatile long pendingRows;
    private volatile long lastFlushDurationMillis;

    LanceDbSinkMetrics(@Nullable SinkWriterMetricGroup metricGroup) {
        if (metricGroup == null) {
            ...
            this.reopenCount = NoOpCounter.INSTANCE;
            return;
        }
        this.recordsOut = metricGroup.getNumRecordsSendCounter();
        this.recordsOutErrors = metricGroup.getNumRecordsOutErrorsCounter();
        this.flushCount = metricGroup.counter("lancedbFlushCount");
        this.flushFailureCount = metricGroup.counter("lancedbFlushFailureCount");
        this.retryCount = metricGroup.counter("lancedbRetryCount");
        this.reopenCount = metricGroup.counter("lancedbReopenCount");
        metricGroup.gauge("lancedbPendingRows", () -> pendingRows);
        metricGroup.gauge("lancedbLastFlushDurationMs", () -> lastFlushDurationMillis);
