...
public class PostgreSQLTableFactoryTest {
        ...
        private static final ResolvedSchema SCHEMA_WITHOUT_PRIMARY_KEY = new ResolvedSchema(
                        Arrays.asList(
                                        Column.physical("aaa", DataTypes.INT().notNull()),
                                        Column.physical("bbb", DataTypes.STRING().notNull()),
                                        Column.physical("ccc", DataTypes.DOUBLE()),
        ...
        private static final boolean SCAN_INCREMENTAL_CLOSE_IDLE_READER_ENABLED_DEFAULT = SCAN_INCREMENTAL_CLOSE_IDLE_READER_ENABLED
                        .defaultValue();
        ...
        public void testMetadataColumns() {
                ...
                expectedSource.metadataKeys = Arrays.asList("row_kind", "op_ts", "database_name", "schema_name",
                                "table_name");

                assertEquals(expectedSource, actualSource);

                ScanTableSource.ScanRuntimeProvider provider = postgreSQLTableSource
                                .getScanRuntimeProvider(ScanRuntimeProviderContext.INSTANCE);
                DebeziumSourceFunction<RowData> debeziumSourceFunction = (DebeziumSourceFunction<RowData>) ((SourceFunctionProvider) provider)
