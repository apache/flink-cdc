...
public class PostgresPartitionInclusionDecider implements Serializable {
    ...
    public boolean isIncluded(TableId tableId) {
        ...
        // Non-partition tables (and partition parents) fall back to direct filter match.
        if (filters.isIncluded(tableId)) {
            return true;
        }

        // Allow partition parents that are configured via partition.tables even when the runtime
        // filter doesn't include them (e.g., when the include list is expanded to partitions).
        return router != null && router.isConfiguredParent(tableId);
    }
