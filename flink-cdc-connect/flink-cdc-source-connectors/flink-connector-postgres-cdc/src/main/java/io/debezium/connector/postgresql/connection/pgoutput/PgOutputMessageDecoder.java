...
/**
 * copied from Debezium 1.9.8-Final with new method readUTF8String Decodes messages from the PG
 * logical replication plug-in ("pgoutput"). See
...
public class PgOutputMessageDecoder extends AbstractMessageDecoder {
    ...
    /**
     * Reads the replication stream up to the next null-terminator byte and return the contents as a
     * utf-8 string
     *
     * @param buffer The replication stream buffer
     * @return string read from the replication stream
    ...
    private static String readUTF8String(ByteBuffer buffer) {
        ...
        // Iterate from current position up to the limit
        while (buffer.hasRemaining()) {
            if (buffer.get() == 0) {
                nullFound = true;
                end = buffer.position() - 1; // The null byte itself
                break;
            }
            end = buffer.position(); // update end if not null yet
        }
        ...
        // if a null terminator was found, advance the buffer's position past it
        if (nullFound) {
            buffer.get();
        }
