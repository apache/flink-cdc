...
/**
 * An abstract common implementation of {@link StreamingChangeEventSource} for binlog-based
 * connectors.
 */
/**
 ...
 *
 * <p>Change 2: {@code handleChange} iterates the row list instead of indexing it — the list may be
 * a linked list, in which case {@code get(i)} turns the loop into O(n^2).
 *
 * <p>Change 3: {@code wrap()} runs the message through {@code ErrorMessageUtils} to keep the error
 * text bounded.
 *
 * <p>Change 4 (OceanBase): skip {@code offsetContext.completeEvent()} for HEARTBEAT events emitted
 * by the OceanBase Binlog Service.
 *
 * <p>Change 5: when the offset carries no GTID set but does pin a binlog file/position, keep the
 * binlog client in file/position mode instead of switching it to GTID mode. Debezium always
 * switches to GTID mode here because upstream only reaches this branch with an empty binlog
 * filename; Flink CDC reaches it whenever a job starts from a specific binlog offset, and there
 * {@code BinaryLogClient} would stream from the GTID set (ignoring the requested position) or, with
 * an empty GTID set, fail with an NPE in {@code DumpBinaryLogGtidCommand} — it only materialises
 * its internal GtidSet when the binlog filename is empty.
 *
 * <p>Debezium 2.7 split the shared binlog engine out of the MySQL connector, so these patches moved
 * here from {@code io.debezium.connector.mysql.MySqlStreamingChangeEventSource}, which is now a
 * thin subclass and no longer forked.
 */
public abstract class BinlogStreamingChangeEventSource<
                P extends BinlogPartition, O extends BinlogOffsetContext>
        implements StreamingChangeEventSource<P, O> {

    private static final Logger LOGGER =
            LoggerFactory.getLogger(BinlogStreamingChangeEventSource.class);
    ...
    public BinlogStreamingChangeEventSource(
    ...
    public void execute(ChangeEventSourceContext context, P partition, O offsetContext)
        ...
        if (getOffsetContext() == null) {
            setEffectiveOffsetContext(offsetContext);
        }
        ...
        if (connectorConfig.getBufferSizeForStreamingChangeEventSource() == 0) {
            ...
            EventBuffer<?, P, O> buffer =
                    new EventBuffer<>(
                            connectorConfig.getBufferSizeForStreamingChangeEventSource(),
                            this,
                            context);
        ...
        if (isGtidModeEnabled) {
            ...
            eventHandlers.put(
                    ...
                    (event) ->
                            handleGtidEvent(
                                    partition, effectiveOffsetContext, event, gtidDmlSourceFilter));
            ...
            if (filteredGtidSet != null) {
                ...
                // from the beginning ...
                final String startBinlogFilename =
                        effectiveOffsetContext.getSource().binlogFilename();
                client.setBinlogFilename(startBinlogFilename);
                client.setBinlogPosition(effectiveOffsetContext.getSource().binlogPosition());
                if (!Strings.isNullOrEmpty(startBinlogFilename)) {
                    LOGGER.info(
                            "No GTID stored in the offset, registering binlog reader with binlog file '{}' and position {}.",
                            startBinlogFilename,
                            effectiveOffsetContext.getSource().binlogPosition());
                    initializeGtidSet("");
                } else if (purgedServerGtidSet == null || purgedServerGtidSet.isEmpty()) {
                    LOGGER.info(
                            "No GTID stored in the offset, registering binlog reader with empty GTID set.");
                    client.setGtidSet("");
                    initializeGtidSet("");
                } else {
                    LOGGER.info(
                            "No GTID stored in the offset, but there is non-empty purged GTID set. Registering binlog reader with purged GTID set: '{}'",
    ...
    protected BinaryLogClient createBinaryLogClient(
        ...
        final long keepAliveInterval =
                configuration.getLong(BinlogConnectorConfig.KEEP_ALIVE_INTERVAL_MS);
        ...
        // Considering heartbeatInterval should be less than keepAliveInterval, use the
        // heartbeatIntervalFactor
        // multiply by keepAliveInterval and set the result value to heartbeatInterval. The default
        // value of
        // heartbeatIntervalFactor is 0.0, and we believe the left time (0.2 * keepAliveInterval) is
        // enough
    ...
    protected EventDeserializer createEventDeserializer() {
        ...
        eventDeserializer.setEventDataDeserializer(
                EventType.TRANSACTION_PAYLOAD,
                new TransactionPayloadDeserializer(
                        tableMapEventByTableId, eventDeserializationFailureHandlingMode));
    ...
    protected void handleEvent(
            P partition, O offsetContext, ChangeEventSourceContext context, Event event) {
        if (event == null) {
            return;
        }
        ...
        try {
            ...
            // Skip HeartBeat event for OceanBase Binlog Service
     ...
     * @param event the server stopped event to be processed; may not be null
     */
    protected void handleServerHeartbeat(P partition, O offsetContext, Event event)
            throws InterruptedException {
     ...
     * new file. In either case, subsequent table numbers will be different from those seen to this
     * point.
     *
     ...
     * statements as changes in the database schemas.
     *
     ...
     */
    protected void handleQueryEvent(P partition, O offsetContext, Event event)
            throws InterruptedException {
        ...
        String upperCasedStatementBegin =
                Strings.getBegin(removeSetStatement(sql), 7).toUpperCase();
        ...
        try {
            for (SchemaChangeEvent schemaChangeEvent : schemaChangeEvents) {
                ...
                if (tableId != null
                        ...
                        && schemaChangeEvent
                                .getType()
                                .equals(SchemaChangeEvent.SchemaChangeEventType.TRUNCATE)) {
                    eventDispatcher.dispatchDataChangeEvent(
                            partition,
                            tableId,
                            new BinlogChangeRecordEmitter<>(
                                    partition,
                                    offsetContext,
                                    clock,
     ...
     */
    protected void handleUpdateTableMetadata(P partition, O offsetContext, Event event)
            throws InterruptedException {
     ...
     */
    protected void handleTransactionPayload(P partition, O offsetContext, Event event)
            throws InterruptedException {
        ...
        for (Event uncompressedEvent : transactionPayloadEventData.getUncompressedEvents()) {
            eventHandlers
                    .getOrDefault(
                            ...
                            (e) -> ignoreEvent(offsetContext, uncompressedEvent))
     ...
     */
    protected void handleInsert(P partition, O offsetContext, Event event)
            throws InterruptedException {
        handleChange(
                partition,
                offsetContext,
     ...
     * @throws InterruptedException if this thread is interrupted while blocking
     */
    protected void handleUpdate(P partition, O offsetContext, Event event)
            throws InterruptedException {
        handleChange(
                partition,
                offsetContext,
     ...
     * @throws InterruptedException if this thread is interrupted while blocking
     */
    protected void handleDelete(P partition, O offsetContext, Event event)
            throws InterruptedException {
        handleChange(
                partition,
                offsetContext,
    ...
    protected void handleTransactionBegin(P partition, O offsetContext, Event event, Long threadId)
            throws InterruptedException {
        Instant eventTime = Conversions.toInstantFromMillis(eventTimestamp.toEpochMilli());
     ...
     */
    protected abstract void handleGtidEvent(
            P partition, O offsetContext, Event event, Predicate<String> gtidSourceFilter)
            throws InterruptedException;
    ...
    private Predicate<String> getGtidDmlSourceFilter() {
        if (connectorConfig
                .getConfig()
                .getBoolean(BinlogConnectorConfig.GTID_SOURCE_FILTER_DML_EVENTS)) {
            return connectorConfig.getGtidSourceFilter();
        }
        return null;
    }

    private void handleTransactionCompletion(P partition, O offsetContext, Event event)
            throws InterruptedException {
     ...
     * ignore that event or raise a warning or error per the {@link
     * BinlogConnectorConfig#INCONSISTENT_SCHEMA_HANDLING_MODE} configuration.
     *
     * @param partition the partition; never null
     * @param offsetContext the offset context; never null
        ...
        if (tableId != null
            ...
            if (tableId == null) {
                ...
                if (eventData instanceof WriteRowsEventData) {
                    tableId =
                            schema.getExcludeTableId(((WriteRowsEventData) eventData).getTableId());
                } else if (eventData instanceof UpdateRowsEventData) {
                    tableId =
                            schema.getExcludeTableId(
     ...
     */
    private void informAboutUnknownTableIfRequired(
            P partition, O offsetContext, Event event, TableId tableId)
            throws InterruptedException {
    ...
    private <T extends EventData, U> void handleChange(
        ...
        if (tableId != null && taskContext.getSchema().schemaFor(tableId) != null) {
            ...
            if (startingRowNumber < numRows) {
                // Iterate rather than index: the row list may be a linked list, in which case
                // rows.get(i) turns this loop into O(n^2).
                int rowIndex = 0;
                for (U row : rows) {
    ...
    private SSLSocketFactory getBinlogSslSocketFactory(
            BinlogConnectorConfig connectorConfig, BinlogConnectorConnection connection) {
