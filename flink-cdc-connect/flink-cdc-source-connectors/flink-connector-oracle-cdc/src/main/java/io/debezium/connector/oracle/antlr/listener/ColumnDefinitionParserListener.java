 ...
 * resulting column has a {@code null} type name. Since Debezium 2.2 the field-name selector always
 * wraps its namer in a {@code FieldNameCache}, whose backing map calls {@link
 * io.debezium.relational.Column#equals(Object)}, and that dereferences {@code typeName()} without a
 * null check -- so such a column throws a {@link NullPointerException} while the table schema is
 * built. Recognise {@code SYS.XMLTYPE} as well, mirroring what the Oracle pipeline connector's own
        ...
        if (ctx.datatype() == null) {
            if (ctx.type_name() != null
                ...
                columnEditor.jdbcType(Types.STRUCT).type("MDSYS.SDO_GEOMETRY");
                    ...
                    && "SYS.XMLTYPE"
                            .equalsIgnoreCase(ctx.type_name().getText().replace("\"", ""))) {
                columnEditor.jdbcType(Types.SQLXML).type("SYS.XMLTYPE");
            }
        } else {
