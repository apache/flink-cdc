...
class PendingSplitsStateSerializerTest {
    ...
    void testSerializeSnapshotPendingSplitsState() throws Exception {
        PendingSplitsStateSerializer serializer =
                new PendingSplitsStateSerializer(constructSourceSplitSerializer());
        PendingSplitsState state = new SnapshotPendingSplitsState(
                ...
                true,
                Collections.emptyMap(),
                new ChunkSplitterState(constructTableId(), ChunkSplitterState.ChunkBound.middleOf(1), 2)
        );

        assertThat(serializer.deserialize(serializer.getVersion(), serializer.serialize(state))).isEqualTo(state);
    }
