...
public abstract class JdbcSourceChunkSplitter implements ChunkSplitter {
  ...
  public Collection<SnapshotSplit> generateSplits(TableId tableId) {
    try (JdbcConnection jdbc = dialect.openJdbcConnection(sourceConfig)) {
      ...
      Table table = Objects.requireNonNull(dialect.queryTableSchema(jdbc, tableId)).getTable();
      ...
      for (int i = 0; i < chunks.size(); i++) {
        ...
        SnapshotSplit split =
            createSnapshotSplit(
                jdbc, tableId, i, splitType, chunk.getChunkStart(), chunk.getChunkEnd());
        splits.add(split);
      }

      long end = System.currentTimeMillis();
      LOG.info(
          "Split table {} into {} chunks, time cost: {}ms.", tableId, splits.size(), end - start);
      return splits;
    } catch (Exception e) {
  ...
  /**
   * Query the maximum value of the next chunk, and the next chunk must be greater than or equal to
   * <code>includedLowerBound</code> value [min_1, max_1), [min_2, max_2),... [min_n, null). Each
   * time this method is called it will return max1, max2...
   *
   ...
   *
   * <p>Each database has different system table to lookup up approximate total number. For example,
   * `pg_class` in postgres, `sys.dm_db_partition_stats` in sqlserver, `SYSCAT.TABLE` in db2.
   *
   * @param jdbc JDBC connection.
   * @param tableId table identity.
  ...
  protected Object[] queryMinMax(JdbcConnection jdbc, TableId tableId, Column splitColumn)
    ...
    return JdbcChunkUtils.queryMinMax(
        jdbc, jdbc.quotedTableIdString(tableId), jdbc.quotedColumnIdString(splitColumn.name()));
  }

  /**
   * Query the minimum value of the column in the table, and the minimum value must greater than the
   * excludedLowerBound value. e.g. prepare query string <code>
   * SELECT MIN(%s) FROM %s WHERE %s > ?</code>
   *
   * @param jdbc JDBC connection.
   * @param tableId table identity.
  ...
  private Object nextChunkEnd(
    ...
    Object chunkEnd = queryNextChunkMax(jdbc, tableId, splitColumn, chunkSize, previousChunkEnd);
