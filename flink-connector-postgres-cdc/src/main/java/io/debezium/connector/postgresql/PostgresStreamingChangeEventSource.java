 ...
 * https://github.com/ververica/flink-cdc-connectors/issues/1893.
 *
 * <p>Add flag {@link PostgresStreamingChangeEventSource#lsnFlushingAllowed} of ensure that LSN is
 * flushed only if we are really in message processing mode. We should remove this class after we
 * bumped 2.0 debezium version where the https://issues.redhat.com/browse/DBZ-5031 has been fixed.
 */
public class PostgresStreamingChangeEventSource
                                ...
                                if (message.isTransactionalMessage()) {
                                    ...
                                    if (message.getOperation() == Operation.BEGIN) {
                                        dispatcher.dispatchTransactionStartedEvent(
                                                Long.toString(message.getTransactionId()),
                                                offsetContext);
    ...
    private void commitMessage(PostgresOffsetContext offsetContext, final Lsn lsn)
            throws SQLException, InterruptedException {
        lastCompletelyProcessedLsn = lsn;
        offsetContext.updateCommitPosition(
                lsn,
                lastCompletelyProcessedLsn,
                null,
                null,
