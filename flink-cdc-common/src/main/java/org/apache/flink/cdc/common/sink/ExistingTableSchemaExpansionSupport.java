...
public interface ExistingTableSchemaExpansionSupport extends Serializable {
     ...
     * produced by {@link #normalizeToTargetDataType(TableId, String, DataType)}.
     */
     ...
     * applier will create in the target system. For example, if both {@code CHAR(n)} and {@code
     * VARCHAR(n)} map to target {@code VARCHAR(n)}, this method should return the same {@code
     * VARCHAR(n)} representation for both input types.
     *
