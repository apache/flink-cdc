...
package org.apache.flink.cdc.tools.ci.licensecheck;
...
public class JarFileChecker {
    ...
    private static int findNonBinaryFilesContainingText(
        ...
        try (Stream<Path> files = Files.walk(jarRoot)) {
            return files.filter(path -> !path.equals(jarRoot))
                    ...
                    .filter(path -> !pathStartsWith(path, "/javax/servlet"))
    ...
    private static int getNumLicenseFilesOutsideMetaInfDirectory(Path jar, Path jarRoot)
        ...
        try (Stream<Path> files = Files.walk(jarRoot)) {
             ...
             *
             * <p>In such a case, we recommend updating the shading exclusions, and adding the license file
             * to META-INF/licenses.
             */
            final List<String> filesWithIssues =
                    files.filter(path -> !path.equals(jarRoot))
                            ...
                            .filter(path -> !Files.isDirectory(path))
                            // some class files contain LICENSE in their name
                            .filter(JarFileChecker::isNoClassFile)
                            // false-positive in Doris and Starrocks
                            .filter(path -> !getFileName(path).endsWith(".ftl"))
                            .map(Path::toString)
                            // license files in META-INF are expected
                            .filter(path -> !path.contains("META-INF"))
                            // false-positive for Microsoft SQL Server
                            .filter(path -> !path.startsWith("/container-license-acceptance"))
                            .collect(Collectors.toList());
